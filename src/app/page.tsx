...
type CheckIn = { checkInTime: string; distanceMeters: number; status: string; branch?: string; alreadyCheckedIn?: boolean };
  ...
  useEffect(() => {
    ...
    fetch("/api/me", {
      ...
      .then(({ ok, body }) => {
        ...
        else setState({ kind: "ready", name: body.employee.fullName, status: body.employee.status });
      })
      ...
      {state.kind === "ready" && (
        <>
          ...
          <section className="actions">
            ...
            <button disabled>Мой график</button>
            <button disabled>Мои проверки</button>
            <button disabled>Мой KPI</button>
          </section>
        </>
      )}
    </main>
