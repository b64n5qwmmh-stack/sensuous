...
export default function HomePage() {
  ...
  return <main className="home">
    ...
    {state.kind === "ready" && <>
      ...
      {panel === "kpi" && <section className="card details"><h2>{t("myKpi")}</h2>{dashboard?.kpi ? <p><strong>{dashboard.kpi.score ?? "—"}%</strong><br />{t("branchKpi")}: {dashboard.kpi.branchScore ?? "—"}%<br />{t("eligible")}: {dashboard.kpi.eligible ? t("yes") : t("no")}</p> : <p>KPI —</p>}<h2 className="penalties-title">{t("penalties")}</h2>{dashboard?.penalties.length ? dashboard.penalties.map((item) => <p key={item.id}><strong>−{item.deduction}% · {item.title}</strong><br />{item.reason}{item.date ? ` · ${new Date(item.date).toLocaleDateString(lang === "az" ? "az-AZ" : "ru-RU")}` : ""}</p>) : <p>{t("noPenalties")}</p>}</section>}
