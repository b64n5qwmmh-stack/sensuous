...
export default function TeamPage() {
  ...
  if (detail) return <main>
    <button className="back" onClick={() => setDetail(null)}>{t("backTeam")}</button>
    <p className="eyebrow">{t("employeeCard")}</p><h1>{detail.employee.name}</h1>
    <section className="card"><p><strong>{lang === "az" ? "Şəxsi KPI" : "Личный KPI"}: {detail.employee.kpi ?? "—"}%</strong><br />{t("branchKpi")}: {detail.employee.branchKpi ?? "—"}%<br />{t("eligible")}: {detail.employee.eligible ? t("yes") : t("no")}</p></section>
    {detail.canAward && <section className="actions compact"><button onClick={() => award("month")}>{t("employeeMonth")}</button><button onClick={() => award("year")}>{t("employeeYear")}</button></section>}
    <h2 className="section-title">{t("checks")}</h2>
    {detail.inspections.length ? detail.inspections.map((inspection) => <section className="card inspection" key={inspection.id}>
      ...
      <p>{t("checkedBy")}: {inspection.inspector} · {inspection.status}</p>
      {openInspection === inspection.id && <div className="answers">{inspection.answers.map((answer, index) => <p key={`${inspection.id}-${index}`}><strong>{index + 1}. {answer.question}</strong><br />{lang === "az" ? "Qiymət" : "Оценка"}: {answer.result}{answer.score !== null ? ` (${answer.score}/5)` : ""}</p>)}</div>}
    </section>) : <section className="card">{t("noChecks")}</section>}
  ...
  return <main>
    ...
    <p className="eyebrow">SENSUM STAFF OS</p><h1>{t("teamRating")}</h1>{message && <section className="card warning">{message}</section>}
    {!team ? <section className="card">Загружаем данные…</section> : <>
      <section className="tabs"><button className={mode === "branches" ? "selected" : ""} onClick={() => setMode("branches")}>{t("branches")}</button><button className={mode === "month" ? "selected" : ""} onClick={() => setMode("month")}>{t("month")}</button><button className={mode === "year" ? "selected" : ""} onClick={() => setMode("year")}>{t("year")}</button></section>
      {mode === "branches" ? team.branches.map((branch) => <section className="card team-branch" key={branch.id}><h2>{branch.name}</h2>{branch.employees.map((person) => <button className="person-row" onClick={() => showEmployee(person.id)} key={person.id}><span>{person.name}</span><strong>{person.kpi ?? "—"}%</strong></button>)}</section>) : <section className="card ranking"><h2>{mode === "month" ? t("monthRating") : t("yearRating")}</h2>{list?.length ? list.map((person) => <button className="person-row" onClick={() => showEmployee(person.id)} key={person.id}><span>#{person.rank} · {person.name}</span><strong>{mode === "month" ? person.kpi : person.yearKpi}%</strong></button>) : <p>{t("noCalculated")}</p>}</section>}
      <LanguageSwitch lang={lang} setLang={setLang} label={t("language")} />
    </>}
  </main>;
}
