...
async function query(input: Parameters<NonNullable<typeof notion>["databases"]["query"]>[0]) {
  if (!notion) return { results: [] };
  try { return await notion.databases.query(input); }
  catch (error) { console.error("Dashboard query failed", error); return { results: [] }; }
}
...
export async function getDashboard(employee: Employee) {
  ...
  const [attendance, inspections, kpi] = await Promise.all([
    process.env.NOTION_ATTENDANCE_DATA_SOURCE_ID ? query({ database_id: process.env.NOTION_ATTENDANCE_DATA_SOURCE_ID, filter: { property: "Employee", relation: { contains: employee.id } }, sorts: [{ property: "Check-in Time", direction: "descending" }], page_size: 7 }) : Promise.resolve({ results: [] }),
    process.env.NOTION_INSPECTIONS_DATA_SOURCE_ID ? query({ database_id: process.env.NOTION_INSPECTIONS_DATA_SOURCE_ID, filter: { property: "Employee Checked", relation: { contains: employee.id } }, sorts: [{ property: "Date", direction: "descending" }], page_size: 10 }) : Promise.resolve({ results: [] }),
    process.env.NOTION_KPI_RESULTS_DATA_SOURCE_ID ? query({ database_id: process.env.NOTION_KPI_RESULTS_DATA_SOURCE_ID, filter: { property: "Employee", relation: { contains: employee.id } }, sorts: [{ property: "Period", direction: "descending" }], page_size: 1 }) : Promise.resolve({ results: [] }),
  ]);

  return {
    attendance: attendance.results.flatMap((page) => {
      if (!("properties" in page)) return [];
      const p = page.properties as Record<string, unknown>; const checkInTime = date(p["Check-in Time"]);
